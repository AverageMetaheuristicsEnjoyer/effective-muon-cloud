...
# Standard causal softmax attention + pure all-Linear Tucker on FineWeb.
# Every independent Linear (Q/K/V/O, gate/up/down, lm_head) uses only Tucker
# factors/core. Scalar rank 259 gives 257,193,298 parameters: +4,434 relative
# to the original 257,188,864 dense control, within the allowed 12,312 gap.
# No tensorized/reconstruction attention and no Tucker residual/filler are used.
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"standard_attention_tucker_r259_pure_muon_lr1e-3_1xC"}
...
torchrun \
    ...
    --wandb-tags bf16 muon streaming standard-attention tucker all-linear "${TUCKER_MODE_TAG}"
